...
function updateId() {
    form.dates.forEach((date) => {
        date.id = form.dates.indexOf(date);
...
function validateWithDB() {
    function isTimeOverlap(formTime, appointmentTime) {
        const formStart = moment(formTime.start, timeFormat);
        const formEnd = moment(formTime.end, timeFormat);

        const appointmentStart = moment(appointmentTime.start, timeFormat);
        const appointmentEnd = moment(appointmentTime.end, timeFormat);

        return (
            formStart.isBefore(appointmentEnd) &&
            formEnd.isAfter(appointmentStart)
        );
    }

    function hasOverlap(formDates, appointments) {
        // حلقه برای بررسی هر قرار در آرایه appointments
        return appointments.some((appointment) =>
            appointment.dates.some((appointmentDate) =>
                formDates.some((formDate) => {
                    // check overlap only for "none" and "weekly" frequencies
                    if (
                        form.frequency !== "custom" &&
                        appointment.frequency !== "custom"
                    ) {
                        if (form.frequency === appointment.frequency) {
                            // if frequencies are same and dates aren't the same, there's no overlap
                            if (formDate.date !== appointmentDate.date) {
                                return false;
                            }
                        }
                        // if frequencies aren't the same, convert both dates to moment and then check for overlap
                            ...
                            const appointmentMoment = moment(
                                ...
                                appointment.frequency === "none"
                                    ? "YYYY-MM-DD"
                                    : "dddd"
                            );
                            // مقایسه روزها
                            if (!formMoment.isSame(appointmentMoment, "day")) {
                                return false;
                            }
                        }
                    }
                    ...
                    return formDate.times.some((formTime) =>
                        appointmentDate.times.some((appointmentTime) =>
                            isTimeOverlap(formTime, appointmentTime)
                        )
                    );
                })
            )
    ...
    const formHasOverlap = hasOverlap(form.dates, appointments);
    if (formHasOverlap) {
                ...
                date.times.forEach((time) => {
                    ...
                    const rrule = new RRule({
                        ...
                        dtstart: moment()
                            ...
                            .toDate(),
                        until: moment()
                            .set({ year: eYear, month: eMonth, date: eDay })
                            .toDate(),
                    });
                    rruleSet.rrule(rrule);
                    time.rrule = rruleSet.toString();
...
const createAppointment = () => {
    createRRule();
    const finalValidation = validateWithDB();
...
<template>
    <section>
        <PrimaryButton class="!rounded-md" @click="confirmDateCreation">
            <span class="px-2">ایجاد</span>
            <PlusIcon class="size-7" />
        </PrimaryButton>
        <Modal :show="confirmingDateCreation" @close="closeModal">
            <div class="p-5">
                <h2 class="text-lg font-medium mb-2 text-gray-900">
                    ایجاد زمان جدید
                </h2>
                <div class="mb-3">
                    <TextInput
